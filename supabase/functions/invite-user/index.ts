...
Deno.serve(async (request) => {
  ...
  const responseOrigin = allowedOrigin === "*" || requestOrigin === allowedOrigin
  ...
  if (!supabaseUrl || !publishableKey || !secretKey || !authorization.startsWith("Bearer ")) {
    return json({ error: "Function authentication is not configured" }, 500, responseOrigin);
  }
  ...
  try {
    ...
    if (profileError || !callerProfile?.is_active || callerProfile.role !== "system_admin") {
      return json({ error: "Administrator permission required" }, 403, responseOrigin);
    }
    ...
    if (inviteError || !inviteData.user) {
      return json({ error: inviteError?.message ?? "Invitation failed" }, 400, responseOrigin);
    }
    ...
    return json({ error: "Unexpected invitation error" }, 500, responseOrigin);
  }
});
